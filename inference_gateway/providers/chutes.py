    ...
    async def init(self) -> "ChutesProvider":
        ...
        for whitelisted_chutes_model in WHITELISTED_CHUTES_INFERENCE_MODELS:
            chutes_model = next(
                (
                    chutes_model
                    for chutes_model in chutes_models
        ...
        self.chutes_client = AsyncOpenAI(base_url=config.CHUTES_BASE_URL, api_key=config.CHUTES_API_KEY)

        return self

    async def _inference(
        self, model_info: ModelInfo, temperature: float, messages: List[InferenceMessage]
    ) -> InferenceResult:
        try:
            chat_completion = await self.chutes_client.chat.completions.create(
                model=model_info.external_name, temperature=temperature, messages=messages, stream=False
            )
            ...
            return InferenceResult(status_code=e.status_code, response=e.response.text)

        except Exception as e:
            return InferenceResult(
                status_code=-1, response=f"Error in ChutesProvider._inference(): {type(e).__name__}: {str(e)}"
            )
